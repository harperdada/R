...
# Squeeze Probability Engine — Last 30 Bars
# ============================
...
n_days <- 60   # how many recent bars to print
...
# --- Full history indicators (needed for accurate rolling calcs) ---
sma50  <- SMA(close, 50)
sma20  <- SMA(close, 20)
rsi14  <- RSI(close, 14)
...
# --- Chop needs full close/sma50 series ---
...
cat(sprintf(
  "%-12s %8s %8s %8s %8s %4s %-10s %6s %8s %8s %6s %-8s %5s %-5s %5s %5s %7s %9s\n",
  "Date","Open","High","Low","Close","RSI","MomInt","ATR%",
  "SQZ","Ratio","Prob%","FireRisk","VCScr","Bias","SqDys","Chop","Ret1Y","Action"
))

cat(paste(rep("-", 185), collapse = ""), "\n")

# --- Index of last n_days ---
n_total  <- NROW(df)
idx_range <- max(1, n_total - n_days + 1):n_total

for (i in idx_range) {

  # Skip if not enough lookback for indicators
  if (i < 55) next
  ...
  sqz_score_i <- round((1 - ratio_i) * 12, 0)
  ...
  # Chop over prior 30 bars ending at i
  ...
  # SQZ_Days at bar i — count consecutive squeeze bars ending at i
  ...
  action_i <- dplyr::case_when(
    choppy_i                                              ~ "AVOID(CHOP)",
    ratio_i < 0.85 & mom_i > 0 & mom_delta_i > 0        ~ "BUY(SQZ FIRE)",
    mom_int_i == "INTACT" & rsi_i < 65                  ~ "BUY",
    mom_int_i == "DECEL"                                 ~ "WATCH",
    TRUE                                                  ~ "DO NOTHING"
  )

  cat(sprintf(
    "%-12s %8.2f %8.2f %8.2f %8.2f %4d %-10s %6.2f %8s %8.2f %6d %-8s %5d %-5s %5d %5.2f %7.2f %9s\n",
...
# ============================================================
# SUMMARY BLOCK — current bar snapshot
# ============================================================
...
last_sqz     <- dplyr::case_when(last_ratio < 0.75 ~ "TIGHT", last_ratio < 1.00 ~ "SQUEEZE", TRUE ~ "RELEASE")
...
last_chop    <- as.numeric(tail(spread_full, 30))
last_chop_sc <- round(sum(diff(sign(as.numeric(last_chop))) != 0, na.rm = TRUE) / 30, 2)
last_prob    <- calc_sqz_prob(last_ratio, vc_score_full, last_sqzdays, sqz_hist_pct, last_mom, last_chop_sc)
