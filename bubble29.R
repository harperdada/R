...
for (ticker in tickers) {
  ...
  out <- tryCatch({
    ...
    # RSI slope (1-day change) + trend label (noise-aware)
    rsi_prev <- as.numeric(tail(na.omit(rsi14), 2))[1]
    rsi_last <- as.numeric(tail(na.omit(rsi14), 2))[2]
    rsi_slope <- if (length(na.omit(rsi14)) >= 2) (rsi_last - rsi_prev) else NA_real_

    rsi_trend <- if (is.na(rsi_slope)) {
      NA_character_
    } else if (rsi_slope > 0.5) {
      "RISING"
    } else if (rsi_slope < -0.5) {
      "FALLING"
    } else {
    ...
    # Optional: smoother slope over ~1 week (5 bars)
    rsi_slope_5d <- if (length(na.omit(rsi14)) >= 6) {
      as.numeric(last(na.omit(rsi14))) - as.numeric(tail(na.omit(rsi14), 6))[1]
    } else {
      NA_real_
    }
    ...
    data.frame(
      ...
      # ✅ NEW: RSI slope + direction
      RSI_Slope_1d = round(rsi_slope, 2),
      RSI_Trend_1d = rsi_trend,
...
if (!is.null(results)) {
  ...
  # header
  cat(sprintf(
    "%-8s %-10s %8s %-12s %8s %8s %5s %8s %-8s %8s %-8s %6s %6s %15s %8s\n",
    "Ticker","Signal","Quality","Status",
    "Price","SMA50","RSI",
    "RSIΔ1d","RSITr1d","RSIΔ5d","RSITr5d",
  ...
  for (i in 1:nrow(results)) {
    ...
    cat(sprintf(
      "%-8s %-10s %8d %-12s %8.2f %8.2f %5d %8.2f %-8s %8.2f %-8s %6.2f %6.2f %15s %8d\n",
      r$Ticker,
      r$Signal,
      r$Quality,
      r$Status,
      ...
      r$RSI,
      r$RSI_Slope_1d,
      ifelse(is.na(r$RSI_Trend_1d), "", r$RSI_Trend_1d),
      r$RSI_Slope_5d,
