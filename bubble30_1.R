...
for (ticker in tickers) {
  ...
  out <- tryCatch({
    ...
    # --- 1Y Return (approx 252 trading days) ---
    close_vec <- as.numeric(close)
    n_close   <- length(close_vec)

    price_1y_ago <- if (n_close >= 253) close_vec[n_close - 252] else NA_real_

   ret_1y_pct <- if (!is.na(price_1y_ago) && price_1y_ago > 0 && !is.na(current_price)) {
       100 * (current_price / price_1y_ago - 1)
   } else {
      NA_real_
   }
...
if (!is.null(results)) {
  ...
  # header
  cat(sprintf(
  "%-8s %-10s %8s %-12s %8s %8s %8s %5s %8s %-8s %6s %6s %15s %8s\n",
  "Ticker","Signal","Quality","Status",
  "Price","SMA50","Ret1Y%","RSI",
  "RSIΔ5d","RSITr5d",
  ...
  for (i in 1:nrow(results)) {
    r <- results[i, ]
    cat(sprintf(
  "%-8s %-10s %8d %-12s %8.2f %8.2f %8s %5d %8.2f %-8s %6.2f %6.2f %15s %8d\n",
  r$Ticker,
  r$Signal,
  r$Quality,
  r$Status,
  r$Price,
  r$SMA50,
  ifelse(is.na(r$Return_1Y), "", sprintf("%.2f", r$Return_1Y)),
  r$RSI,
  r$RSI_Slope_5d,
