...
# --- 1. LOAD TICKERS ---
# Reading a single-column CSV with no header
...
for (ticker in tickers) {
  ...
  out <- tryCatch({
    ...
    #current_atrpct <- 100 * current_atrd / current_price
...
perfect_cc <- (
  ...
  #SafeStrike = round(safe_strike, 0),
  #CC_DTE     = "WEEKLY",
  #TooSoonCC = ifelse(too_soon_cc, "⛔ TOO SOON", ""),
...
if (!is.null(results)) {
  ...
  # DEBUG: show columns so we know ATRD exists
...
cat("  • If premium < 0.3% of notional → skip\n\n")
  ...
  # header
 cat(sprintf(
  "%-8s %-10s %8s %-12s %8s %8s %5s %6s %6s %15s %8s\n",
  "Ticker","Signal","Quality","Status",
  "Price","SMA50","RSI","ATRD","ATR%","CCFlag","Fake_30d"
))
cat(paste0(rep("-", 112), collapse=""), "\n")
  ...
  cat(sprintf(
    "%-8s %-10s %8d %-12s %8.2f %8.2f %5d %6.2f %6.2f %15s %8d\n",
    r$Ticker,
    r$Signal,
    r$Quality,
    r$Status,
    ...
    r$RSI,
    r$ATRD,
    r$ATRPct,      # <-- column name in your df
    r$CCFlag,
    r$Fake_30d
  ))
}
